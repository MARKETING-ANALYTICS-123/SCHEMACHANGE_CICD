CREATE OR REPLACE PROCEDURE "USP_POPULATE_RETENTION_12M_MADEWELL"("START_DATE" ARRAY, "END_DATE" ARRAY, "FISCAL_YEAR" NUMBER(38,0), "FISCAL_MONTH" NUMBER(38,0))
RETURNS VARCHAR(20)
LANGUAGE SQL
EXECUTE AS OWNER
AS '
...
INSERT INTO RPT."MADEWELL RETENTION L12"
(
    "MasterCustomerId",
    "Brand",
    "Transaction Id",
...
WITH MWLAGDATA AS (
    ...
    (
        SELECT
            ...
            COUNT(1)
            FROM
            loyalty.mw_prd.prd_mw_customers_snapshot
            WHERE date(snapshot_date) = :END_DATE[2]) <> 0 THEN :END_DATE[2] ELSE (SELECT MIN(SNAPSHOT_DATE) FROM loyalty.mw_prd.prd_mw_customers_snapshot) END AS snapshot_date
    ),
    ...
    (
        SELECT
            ...
            FROM
            loyalty.mw_prd.prd_mw_customers_snapshot
            WHERE date(snapshot_date) = :END_DATE[1]) <> 0 THEN DATEADD(DAY, 1, :END_DATE[1]) ELSE (SELECT MIN(SNAPSHOT_DATE) FROM loyalty.mw_prd.prd_mw_customers_snapshot) END AS snapshot_date
    ),
    ...
    Get_Tier AS (
            ...
            FROM (
                SELECT
                    ...
                    CASE 
                        WHEN c.top_tier_name = ''Insider'' THEN 1
                        WHEN c.top_tier_name = ''Star'' THEN 2
                        WHEN c.top_tier_name = ''Icon'' THEN 3
                        WHEN c.top_tier_name = ''Icon+'' THEN 4
                    END AS tier_sort
           ...
           --  "Loyalty Tier"
           --  FROM EDW_MARKETING_PRD.RPT."MADEWELL LOYALTY TIER" TS
           --  INNER JOIN
           --  (
           --      SELECT FISCALYEAR,
           ...
           --          AND DATE(TT.TRANSACTIONDATE) >= DATE(:START_DATE[0])
           --          AND DATE(TT.TRANSACTIONDATE) <= DATE(:END_DATE[0])
        			-- GROUP BY ALL
           ...
           --          LEFT JOIN CDP.PUBLIC.TIMESUMMARY TS ON DATE(TT.TRANSACTIONDATE) = DATE(TS.DATEINUNIX)
           --          WHERE TT.SUBTYPE in (''Demand'', ''Shipped'')
           --          AND DATE(TT.TRANSACTIONDATE) >= DATE(:START_DATE[1])
           --          AND DATE(TT.TRANSACTIONDATE) <= DATE(:END_DATE[1])
        			-- GROUP BY ALL
           --  ) T ON TS."MasterCustomer ID" = T."MasterCustomerId" AND T."MasterCustomer Transaction Sequence Number" = TS."MasterCustomer Transaction Sequence"
        ...
        Get_Customer_Tier AS (
            SELECT
                DISTINCT
                GMT."MasterCustomerId" AS "MasterCustomerId",
                GMT.period,
                CASE WHEN "Loyalty Tier" = ''Insider'' THEN 1
                    WHEN "Loyalty Tier" = ''Star'' THEN 2
                    WHEN "Loyalty Tier" = ''Icon'' THEN 3
                    WHEN "Loyalty Tier" = ''Icon+'' THEN 4
                    ELSE 5 END AS "Loyalty Tier"
            FROM
                Get_Max_Transaction_Date GMT
            LEFT JOIN Get_Tier GT 
                ON GT."MasterCustomer ID" = GMT."MasterCustomerId" 
                AND GT.Period = GMT.Period
        ),
    ...
    (
       SELECT 
        ...
        LEFT JOIN
            CDP.PUBLIC.ORGANIZATIONSUMMARY O ON O.ID = T.ORGANIZATIONID
        WHERE
    ...
    (
        ...
        FROM
            ...
            LEFT JOIN Final_Channel C ON C."MasterCustomerId" = M."MasterCustomerId" and C.period = M.period
            LEFT JOIN Final_Channel LC ON LC."MasterCustomerId" = M."MasterCustomerId" and LC.period = ''TY'' and M.period = ''LY''
            LEFT JOIN Final_fp P ON P."MasterCustomerId" = M."MasterCustomerId" AND P.period = M.period
            LEFT JOIN Final_fp LP ON LP."MasterCustomerId" = M."MasterCustomerId" and LP.period = ''TY'' and M.period = ''LY''
            LEFT JOIN First_Status_Type FST ON FST."MasterCustomerId" = M."MasterCustomerId" AND FST.period = M.period
            LEFT JOIN First_Status_Type LYS ON LYS."MasterCustomerId" = M."MasterCustomerId" AND ((LYS.period = ''LY'' AND M.period = ''TY'') OR (LYS.period = ''LLY'' AND M.period = ''LY''))
            GROUP BY ALL
    )
