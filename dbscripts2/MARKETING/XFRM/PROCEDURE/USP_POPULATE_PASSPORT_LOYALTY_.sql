CREATE OR REPLACE PROCEDURE "USP_POPULATE_PASSPORT_LOYALTY"()
RETURNS VARCHAR(100)
LANGUAGE SQL
EXECUTE AS OWNER
AS '
...
BEGIN
    ...
    CREATE OR REPLACE TABLE Temp."PASSPORT LOYALTY" (
    ...
    INSERT INTO Temp."PASSPORT LOYALTY" (
        "Loyalty Tier",
	...
	TRANSFIL AS (
        ...
        FROM 
            ...
            WHERE TO_DATE(TS.TRANSACTIONTIMESTAMP) >= (
                SELECT
                    TO_DATE(DATEINUNIX)
                FROM
                    CDP_JC.PUBLIC.TIMESUMMARY
                WHERE
                    FISCALYEAR = (
                        SELECT
                            FISCALYEAR
            ...
            AND TO_DATE(TS.TRANSACTIONTIMESTAMP) <= (
                SELECT
                    MAX(TS.DATEINUNIX::DATE)
                FROM
   ...
   JCLAGDATA AS (
        SELECT
            ...
            CASE
                ...
                WHEN T.ID < 20230928 AND TS.C_REWARDSREDEEMEDFLAG = ''Y'' THEN ''Reward''
                ELSE ''No Reward''
            END AS "Reward Flag",
            CASE
        ...
        FROM
            CDP_JC.PUBLIC.CUSTOMERSUMMARY C
            ...
            LEFT JOIN CDP_JC.PUBLIC.TIMESUMMARY T ON DATE(TS.TRANSACTIONDATE) = DATE(T.DATEINUNIX)
            LEFT JOIN CDP_JC.PUBLIC.TIMESUMMARY TL ON DATE(C.C_LOYALTYSIGNUPDATE) = DATE(TL.DATEINUNIX)
        ...
        TS.TRANSACTIONID AS "Promo Demand Transaction",
        9 AS "Loyalty Tier Week",
        9 AS "Loyalty Tier Month",
        9 AS "Loyalty Tier Quarter",
        9 AS "Loyalty Tier Year",
        9 AS "Loyalty Tier LY Same",
        CASE C_FPMDITEM WHEN ''F'' THEN 1 WHEN ''P'' THEN 2 WHEN ''M'' THEN 3 END AS "Price Type",
        CASE WHEN ts.c_plccflag IN (''1'', ''3'') THEN 1 ELSE 0 END AS "JCCC Transaction Flag"
    FROM
        CDP_JC.PUBLIC.CUSTOMERSUMMARY C
        LEFT JOIN CDP_JC.PUBLIC.TRANSACTIONSUMMARY TS ON TS.MASTERCUSTOMERID = C.MASTERCUSTOMERID
        LEFT JOIN CDP_JC.PUBLIC.TIMESUMMARY T ON DATE(TS.TRANSACTIONDATE) = DATE(T.DATEINUNIX)
        LEFT JOIN CDP_JC.PUBLIC.ORGANIZATIONSUMMARY O ON O.ID = TS.ORGANIZATIONID
        LEFT JOIN RPT."ORGANIZATION SUMMARY" OO ON OO.ORGANIZATIONID = TS.ORGANIZATIONID
        LEFT JOIN CDP_JC.PUBLIC.TIMESUMMARY TL ON DATE(C.C_LOYALTYSIGNUPDATE) = DATE(TL.DATEINUNIX)
    ...
    WHERE
        ...
        OR TO_DATE(TS.TRANSACTIONTIMESTAMP) > (
            SELECT
                MAX(TS.DATEINUNIX::DATE)
            FROM
                CDP_JC.PUBLIC.TIMESUMMARY TS
...
DROP TABLE IF EXISTS RPT."PASSPORT LOYALTY";

ALTER TABLE Temp."PASSPORT LOYALTY"  RENAME TO RPT."PASSPORT LOYALTY";

RETURN ''Table RPT."PASSPORT LOYALTY" created or replaced successfully.'';
