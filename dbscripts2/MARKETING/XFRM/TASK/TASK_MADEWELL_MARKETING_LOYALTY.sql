create or replace task TASK_MADEWELL_MARKETING_LOYALTY
	warehouse=MARKETING_PRD_XS_WH
	after EDW_MARKETING_PRD.XFRM.TASK_MADEWELL_MARKETING
	as DECLARE
        V_QUERY_TIME TIMESTAMP;
        V_LAST_MODIFIED_TIME TIMESTAMP;
        V_LAST_MODIFIED_LOYALTY_TIME TIMESTAMP;
        V_TABLE_NAME STRING := 'MADEWELL MARKETING LOYALTY';
...
-- Get the table's last modified time by calling stored procedure
    V_LAST_MODIFIED_TIME := (
        ...
        WHERE
            ...
            AND TABLE_NAME = 'MADEWELL MARKETING LOYALTY'
    );
    
    
    V_LAST_MODIFIED_LOYALTY_TIME := (
        SELECT
            CAST(MAX(last_altered) AS TIMESTAMP)
        FROM
            INFORMATION_SCHEMA.TABLES
    ...
    IF(V_QUERY_TIME > V_LAST_MODIFIED_TIME AND V_LAST_MODIFIED_LOYALTY_TIME > V_LAST_MODIFIED_TIME AND V_QUERY_STATUS = 'SUCCESS') THEN
        CALL XFRM.USP_POPULATE_MADEWELL_MARKETING_LOYALTY();
        CALL EDW_MARKETING.XFRM.USP_POPULATE_BVT_VALIDATION('MADEWELL MARKETING');
    END IF;
END;