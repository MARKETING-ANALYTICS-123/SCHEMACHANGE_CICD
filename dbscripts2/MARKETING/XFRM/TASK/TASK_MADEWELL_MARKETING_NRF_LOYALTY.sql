create or replace task TASK_MADEWELL_MARKETING_NRF_LOYALTY
	warehouse=MARKETING_PRD_XS_WH
	after EDW_MARKETING_PRD.XFRM.TASK_MADEWELL_MARKETING_NRF
	as DECLARE
        V_QUERY_TIME TIMESTAMP;
        V_LAST_MODIFIED_TIME TIMESTAMP;
        V_LAST_MODIFIED_LOYALTY_TIME TIMESTAMP;
        V_TABLE_NAME STRING := 'MADEWELL MARKETING NRF LOYALTY';
    ...
    V_LAST_MODIFIED_TIME := (
        ...
        WHERE
            ...
            AND TABLE_NAME = 'MADEWELL MARKETING NRF LOYALTY'
    );
    ...
    IF(V_QUERY_TIME > V_LAST_MODIFIED_TIME AND V_LAST_MODIFIED_LOYALTY_TIME > V_LAST_MODIFIED_TIME AND V_QUERY_STATUS = 'SUCCESS') THEN
        CALL XFRM.USP_POPULATE_MADEWELL_MARKETING_NRF_LOYALTY();
    END IF;
END;