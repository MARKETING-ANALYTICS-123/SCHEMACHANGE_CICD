create or replace task TASK_MADEWELL_COUPON_STATS_AND_USAGE
...
BEGIN 
  ...
  V_LAST_MODIFIED_TIME := (
    ...
    WHERE
        ...
        AND TABLE_NAME = 'MADEWELL COUPON STATS AND USAGE'
  );
  ...
  V_SNAPSHOT_DATE := (
    ...
    FROM 
      ...
      INNER JOIN cdp.PUBLIC.timesummary t ON DATE(TS.dateinunix) = DATE(t.dateinunix) + 1 
    WHERE 
      TS.FISCALYEAR = (
  ...
  THEN 
    CALL XFRM.USP_POPULATE_MADEWELL_COUPON_STATS_AND_USAGE();
  END IF;
