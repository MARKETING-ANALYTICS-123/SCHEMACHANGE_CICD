create or replace view VW_MKT_TRANSACTIONSUMMARY_PASSPORT_LOYALTY_LEAP(
	"MasterCustomerId",
	"FM Status Type",
	"FQ Status Type",
	"FY Status Type",
...
,C_STATUSTYPEBRAND AS "Status Type"
...
,CASE
    WHEN "Channel Sub Type" = 'AFFILIATES' AND O.LEVEL1NAME = 'Factory' AND C_INITIATIVE IN('(NOT SET)', 'TRUE') THEN '(Other)' 
    WHEN "Channel Sub Type" = 'AFFILIATES' AND O.LEVEL1NAME = 'Factory' AND C_INITIATIVE IN('SHOPPING', 'SHPPING & CONTENT') THEN 'Shopping' 
    WHEN "Channel Sub Type" = 'PAID SEARCH' AND O.LEVEL1NAME = 'Factory' AND (C_INITIATIVE IS NULL OR C_INITIATIVE IN ('(NOT SET)', 'FACTORY-US-KIDS_GENERAL-B-BMM', 'G', 'Y') OR C_INITIATIVE ILIKE 'XPRODU_%') THEN '(Other)'
...
,NULL AS "Loyalty Tier Week Status"
,NULL AS "Loyalty Tier Month Status"
,NULL AS "Loyalty Tier Quarter Status"
,NULL AS "Loyalty Tier Year Status"
...
LEFT JOIN EDWPRD.RPT."Date Exp" DX ON DX."Date Key" = TS.ID
...
LEFT JOIN EDW_MARKETING_PRD.ANALYTICS.VW_MKT_PRODUCTCATEGORYSUMMARY PO 
ON PO."Product Category Level 1" = PS.LEVEL1NAME 
AND PO."Product Category Level 3" = PS.LEVEL3NAME 
AND PO."Product Category Level 2" = PS.LEVEL2NAME 
AND PO."Product Category Level 4" = 
(
	CASE WHEN PS.LEVEL3NAME = 'W PANTS' AND PS.LEVEL4NAME <> 'W PANTS DENIM' THEN 'W PANTS NON DENIM'
	WHEN PS.LEVEL3NAME = 'M PANTS' AND (PS.LEVEL4NAME = 'MENS PANTS DENIM' OR PS.LEVEL4NAME = 'M PANTS DENIM') THEN 'M PANTS DENIM'
	WHEN PS.LEVEL3NAME = 'M PANTS' AND PS.LEVEL4NAME <> 'MENS PANTS DENIM' AND PS.LEVEL4NAME <> 'M PANTS DENIM' THEN 'M PANTS NON DENIM'
	ELSE PS.LEVEL4NAME END
)
AND PO."Product Category Level 5" = PS.LEVEL5NAME
LEFT JOIN CDP_JC.PUBLIC.PRODUCTSUMMARY P ON P.ID = T.PRODUCTID
