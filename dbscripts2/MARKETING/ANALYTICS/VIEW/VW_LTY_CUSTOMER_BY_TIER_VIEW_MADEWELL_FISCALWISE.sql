create or replace view VW_LTY_CUSTOMER_BY_TIER_VIEW_MADEWELL_FISCALWISE(
	"Date Key",
	"Brand Name",
	"MasterCustomer ID",
...
WITH
  ...
  MWLAGDATA AS (
    ...
    WHERE
      ts.mastercustomerid <> '-1'
      AND TO_DATE(TRANSACTIONTIMESTAMP) >= (SELECT TO_DATE(DATEINUNIX) FROM CDP.PUBLIC.TIMESUMMARY WHERE FISCALYEAR = (SELECT FISCALYEAR FROM CDP.PUBLIC.TIMESUMMARY WHERE TO_DATE(DATEINUNIX) > current_date-7 AND TO_DATE(DATEINUNIX) <= current_date AND FISCALDAYOFWEEK = 7)-2 AND FISCALDAYOFMONTH = 1 AND FISCALMONTH = 1)
...
GET_FISCAL_YEAR AS (
    SELECT FISCALYEAR FROM CDP_JC.PUBLIC.TIMESUMMARY
    WHERE TO_DATE(DATEINUNIX) > current_date-7 
        AND TO_DATE(DATEINUNIX) <= current_date 
        AND FISCALDAYOFWEEK = 7
    UNION ALL
    SELECT FISCALYEAR - 1 FROM CDP_JC.PUBLIC.TIMESUMMARY
    WHERE TO_DATE(DATEINUNIX) > current_date-7 
        AND TO_DATE(DATEINUNIX) <= current_date 
        AND FISCALDAYOFWEEK = 7
),
...
GET_LAST_DATE AS (
    select DATE(VALUE) AS LASTDATE from cdp.public.a1_status a where category = 'pipeline' and source = 'sf-bi' and key = 'end_time'and current_timestamp > VALUE and value not in ('0')
),
