create or replace view VW_LTY_CUSTOMER_BY_TIER_VIEW_PASSPORT_FISCALWISE(
	"Date Key",
	"Brand Name",
	"MasterCustomer ID",
...
WHERE 
ts.mastercustomerid <> '-1'
AND TO_DATE(TRANSACTIONTIMESTAMP) >= (SELECT TO_DATE(DATEINUNIX) FROM CDP.PUBLIC.TIMESUMMARY WHERE FISCALYEAR = (SELECT FISCALYEAR FROM CDP_JC.PUBLIC.TIMESUMMARY WHERE TO_DATE(DATEINUNIX) > current_date-7 AND TO_DATE(DATEINUNIX) <= current_date AND FISCALDAYOFWEEK = 7)-2 AND FISCALDAYOFMONTH = 1 AND FISCALMONTH = 1)
...
GET_SUMMARIZED_DATA_WEEK AS (
    SELECT 
        ...
        FROM CDP_JC.PUBLIC.TRANSACTIONSUMMARY TS
        LEFT JOIN CDP_JC.PUBLIC.TIMESUMMARY TT ON DATE(TS.TRANSACTIONDATE) = DATE(TT.DATEINUNIX)
        LEFT JOIN EDW_MARKETING_PRD.ANALYTICS.VW_MKT_ORGANIZATIONSUMMARY VO ON VO."Id" = TS.ORGANIZATIONID
